...
- **State 3 (Working)**: calls `POST /api/recipe/[id]/tweak`, which returns a **Tweak Patch** (only the changed fields, not the whole recipe — ADR-0010); client merges via `applyTweakPatch`. The wait (~10–15s, a single buffered call) shows a **perceived-progress** affordance (`TweakProgress`): quiet loading dots for the first ~2s, then the shared **segmented indicator** (staged Ah Mah voice-lines "Reading your recipe…" → "Almost there…" over segments that fill left→right and hold, the leading segment pulsing — "almost there", never completing). The result landing is the real completion signal. Sub-2s tweaks show only dots, so the segments never flash. Both cues are time-driven; there's no token-level signal to stream.
...
- **Shared dots→segmented-progress loader** (Jun 2026): the chat (`ChatLoader`) and Tweak Bench (`TweakProgress`) share one loader vocabulary in `src/features/shared/components/loaders/` — `usePhaseAfter` (timed phase flip), `useCyclingIndex` + `SegmentedProgress`/`SegmentLines`/`SegmentBar` (advancing voice-line over fill-and-hold segments — never looping; the leading segment pulses for "almost there" anticipation), and `LoadingDots`. Both show dots first, then the segmented indicator after **~2s** (chat lowered from 6s, so the indicator now appears on nearly every generation). The Tweak Bench's old easing bar was retired in favour of the segmented indicator.
