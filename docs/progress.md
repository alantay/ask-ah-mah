...
- **Per-step ingredient quantities**: each `RecipeStep` gains an optional `uses: { name, amount?, unit?, text? }[]` array — the model's own partial quantity for what that step adds, rendered as a quiet chip row (`StepUses`) separate from the step's prose. Numeric amounts scale with the servings stepper (`scaleAmount`, same `servings/baseServings` ratio as the master ingredient list); free-text amounts ("remaining", "to taste") render unscaled.
- Shows up on all three recipe surfaces that render steps: the recipe page Method section and chat `RecipeLetter` (via shared `StepItem`/`StepList`), and CookingMode (its own local step renderer, via the same standalone `StepUses` component). Also included in the plain-text "Copy recipe" export and extracted when parsing pasted recipe text.
