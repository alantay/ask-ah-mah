...
- **Ah Mah's picking wisdom**: tap a fresh item's name on the recipe shortfall card to reveal "Ah Mah says: [tip]" — practical hints for selecting quality produce at market (e.g., "Pick tomatoes that yield slightly to pressure"). Tips are LLM-generated via `gpt-5-mini` and cached in a shared, user-agnostic `MarketTip` table keyed by canonical item name (see CONTEXT.md "Market Tip" glossary and [ADR-0013](./adr/0013-market-tips-caching.md)).
