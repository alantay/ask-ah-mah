...
- **Read-only reuse**: `RecipeDisplay` gained a `readOnly` prop that hides every owner action (Share, Tweak, the Tweak bench, Start cooking, Back) but keeps **Copy recipe** — useful for whoever opens the link. The public page wraps it (`PublicRecipeView`). _(The original slim brand bar was later dropped — see "Public share page: lead with the recipe" below.)_
...
### Public share page: lead with the recipe, close with an invitation — Shipped Jul 2026 (#402)

- **Turned the shared page into an intro to the app.** A shared `/r/[token]` link previously had zero conversion affordance — `readOnly` stripped every CTA, so a recipe was a dead end. It now funnels a first-time visitor into Ah Mah.
- **Dropped the top brand bar; the recipe photo leads.** `PublicRecipeView`'s slim "Ask Ah Mah" header is gone, so a shared link reads like a recipe someone passed you rather than a web app. Brand isn't lost — the card's existing "FROM AH MAH" byline orients a cold visitor. The now-empty read-only nav row is suppressed, and **Copy recipe** moves to a subtle round button floating over the hero's top-right corner.
- **Signature closing band.** A warm "From Ah Mah" invitation in her voice — granny mark, a Fraunces "Your turn at the stove" headline, and a terracotta **"Cook with what you have →"** CTA to `/` — renders after the recipe on the `--primary-tint` surface with the `.paper` texture. One deliberate element carrying brand + the way into the app, reached right when the reader has finished the recipe.
- **Seam**: `RecipeDisplay` gained an optional `footerSlot?: ReactNode` rendered inside the scroll container after the recipe card; only `PublicRecipeView` passes it (the closing band), so the authenticated `/(app)/recipe/[id]` view is untouched. Marketing copy stays out of the shared `RecipeDisplay`.
...
- **Frontend data-access consolidation** (#424, seam 3, Jul 2026): SWR-backed reads/writes had drifted into per-call-site duplication across three layers. (1) **Key factory** (`src/lib/swr/keys.ts`) — every `/api/*?userId=` (plus message/conversation and market/storage tip) cache key now builds off one factory instead of a re-derived string literal at each `useSWR`/`mutate` call site, so a mutation can no longer revalidate a slightly different string than a reader subscribed to. (2) **Shared mutate helper** (`src/lib/swr/mutateResource.ts`) — lifted from ShoppingList's local `mutateList`, it centralizes the JSON fetch + optional optimistic cache write; adopted at Inventory's `removeItem`, RecipeList's `deleteRecipe`, and the chat recipe card's `addToShoppingList`. Response handling (toast text, revalidate-always vs. revalidate-on-success-only) stays with each caller since it diverges too much to share safely — `MessageList`/`RecipeDisplay`'s multi-step recipe-save flows were left untouched as out of scope. (3) **Unified tips hook** (`src/hooks/useTips.ts`) — `useMarketTips`/`useStorageTips` were ~90%-identical twins (endpoint, key prefix, item field `category` vs `type`, market-only `isPickableCategory` filter); both are now one-line wrappers over a single parameterized `useTips(kind, items, enabled)`, preserving the hour-long dedupe, `keepPreviousData`, and enabled-gating behavior. The key factory's `marketTipKey`/`storageTipKey` were folded onto a shared `tipKey(prefix, names)` builder to match.
- **Tip-corpus engine consolidated across three call sites** (#424, Jul 2026): Market Tips (`POST /api/market-tip`), Storage Tips (`POST /api/storage-tip`), and shopping-list aisle classification (`classifyAisles`, `src/lib/shoppingList/classify.ts`) all repeated the same canonicalize → cache-read → batch-generate misses → cache-write skeleton with only the cache table, prompt, pre-filter, and fallback label differing. That skeleton now lives once in `src/lib/tipCorpus/engine.ts` (`runTipCorpus` + `TipCorpusAdapter`); each call site shrank to request parsing + a small adapter (canonical key, optional Prisma read/write cache, optional pre-filter, one batched `generateObject` call) + response shaping. `classifyAisles` keeps its exported signature unchanged, and both tip routes keep byte-for-byte request/response contracts (auth via `withAuth`, negative-caching, omission handling). No behavior change — same existing tests (`market-tip/route.test.ts`, `storage-tip/route.test.ts`, `shoppingList/classify.test.ts`) pass unmodified.
- **Sign-in value nudges at two high-investment moments** (#398, Jul 2026): guests get a one-time toast surfacing the sign-in payoff ("your kitchen follows you") at the two moments they've just invested effort — a first recipe save, and the first "I made this" (finish moment) — instead of a persistent banner or gate. Each nudge fires at most once per browser (`src/lib/signInNudge.ts`, a `localStorage` flag per key, same private-mode-safe try/catch pattern as `useTipsPreference`), and the two nudges are **independent**: dismissing/seeing one doesn't suppress the other. Delivered as a `sonner` toast with a "Sign in" action button (not a second toast, not an inline banner) that opens `SignInDialog`, which gained optional `open`/`onOpenChange` props so it can be driven externally while still defaulting to its own trigger button for existing callers (`AuthButton`). Signed-in users and repeat saves/cooks get the existing plain success toast unchanged. Full design → `docs/superpowers/specs/2026-07-09-signin-value-nudges-design.md`.
