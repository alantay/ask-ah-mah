...
- **Instant cold return via persistent SWR cache + optimistic identity** (Jul 2026): reopening the app mid-cook after the OS evicted the tab was slow — nothing painted until the `/api/auth/get-session` round-trip *and then* the data fetches finished in series (SWR's cache was in-memory only). Added a global `SWRProvider` giving SWR a localStorage-backed cache (`src/lib/swr-cache.ts`, hydrated on init, persisted on `visibilitychange → hidden`/`pagehide`), plus an **optimistic persisted `userId`** in `useSession` so cached data paints before the session round-trip settles. The persisted id is **display-only** — the server still trusts only the session cookie (`src/lib/session.ts`); on an identity mismatch (expired anon session, sign-in upgrade) the cache is evicted, and sign-out clears both localStorage keys. Loading gates in `RecipeList` and `recipe/[id]/page.tsx` now treat "identity still resolving" as loading (also fixes a pre-existing "can't find that one" cold-load flash). A **service worker (offline + cached shell) is deferred as a future enhancement** — out of scope for the cold-reload-latency problem this solves. Full rationale → [ADR-0023](./adr/0023-instant-return-uses-optimistic-cached-identity.md).
