...
- **Still buffered.** The patch is smaller than the full echo, so we keep `generateText` and the `finishReason === "length"` 422. Truncation is now far less likely *and* still handled cleanly. (Measured on a one-array "less spicy" tweak, Jun 2026: patch ~420–483 output tokens vs the full echo ~677, ~8.8s vs ~10.3s end-to-end. The model's generation floor — not output size — dominates the wait; the patch trims output and truncation risk, not the multi-second latency.)
